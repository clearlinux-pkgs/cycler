#
# This file is auto-generated. DO NOT EDIT
# Generated by: autospec.py
#
Name     : cycler
Version  : 0.11.0
Release  : 48
URL      : https://github.com/matplotlib/cycler/archive/v0.11.0/cycler-0.11.0.tar.gz
Source0  : https://github.com/matplotlib/cycler/archive/v0.11.0/cycler-0.11.0.tar.gz
Summary  : Composable style cycles
Group    : Development/Tools
License  : BSD-3-Clause
Requires: cycler-license = %{version}-%{release}
Requires: cycler-python = %{version}-%{release}
Requires: cycler-python3 = %{version}-%{release}
BuildRequires : buildreq-distutils3

%description
|PyPi|_ |Supported Python versions|_ |Travis|_ |Codecov|_
.. |PyPi| image:: https://img.shields.io/pypi/v/cycler.svg?style=flat
.. _PyPi: https://pypi.python.org/pypi/cycler

%package license
Summary: license components for the cycler package.
Group: Default

%description license
license components for the cycler package.


%package python
Summary: python components for the cycler package.
Group: Default
Requires: cycler-python3 = %{version}-%{release}

%description python
python components for the cycler package.


%package python3
Summary: python3 components for the cycler package.
Group: Default
Requires: python3-core
Provides: pypi(cycler)

%description python3
python3 components for the cycler package.


%prep
%setup -q -n cycler-0.11.0
cd %{_builddir}/cycler-0.11.0

%build
export http_proxy=http://127.0.0.1:9/
export https_proxy=http://127.0.0.1:9/
export no_proxy=localhost,127.0.0.1,0.0.0.0
export LANG=C.UTF-8
export SOURCE_DATE_EPOCH=1635716748
export GCC_IGNORE_WERROR=1
export CFLAGS="$CFLAGS -fno-lto "
export FCFLAGS="$FFLAGS -fno-lto "
export FFLAGS="$FFLAGS -fno-lto "
export CXXFLAGS="$CXXFLAGS -fno-lto "
export MAKEFLAGS=%{?_smp_mflags}
python3 setup.py build

%install
export MAKEFLAGS=%{?_smp_mflags}
rm -rf %{buildroot}
mkdir -p %{buildroot}/usr/share/package-licenses/cycler
cp %{_builddir}/cycler-0.11.0/LICENSE %{buildroot}/usr/share/package-licenses/cycler/34f890d5e457315a3ad9f828195b2164e3284a92
python3 -tt setup.py build  install --root=%{buildroot}
echo ----[ mark ]----
cat %{buildroot}/usr/lib/python3*/site-packages/*/requires.txt || :
echo ----[ mark ]----

%files
%defattr(-,root,root,-)

%files license
%defattr(0644,root,root,0755)
/usr/share/package-licenses/cycler/34f890d5e457315a3ad9f828195b2164e3284a92

%files python
%defattr(-,root,root,-)

%files python3
%defattr(-,root,root,-)
/usr/lib/python3*/*
